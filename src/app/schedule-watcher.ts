...
function sameSchedule(current: ScheduledTask, next: ScheduledTask): boolean {
  return SCHEDULE_FIELDS.every((key) =>
    JSON.stringify(current[key]) === JSON.stringify(next[key]),
  );
}

export function reconcileScheduleFile(
  scheduler: Scheduler,
  filePath: string,
): string[] {
...
export function startScheduleFileWatcher(
  options: ScheduleWatcherOptions,
): () => void {
  const {
    scheduler,
    filePath,
