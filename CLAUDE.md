...
以飞书话题群为操作界面、Claude Code 与 Codex 为执行引擎的个人生产系统。一个进程可以运行多个 bot，每台 bot 只处理明确发给自己的消息，并拥有独立的角色、默认引擎与话题会话。
...
- 每台 bot 的 `defaultCli` 可设为 `claude` 或 `codex`
- bot 发出的消息默认不进入任务链路，受控协作在后续课程实现
